...
def automatic_configure_model_parameters(model_name=None, new_parameters=None):
    ...
    Returns:
        dict: The updated configuration dictionary for the model.
    """
    try:
        ...
        supported_models = {
            model: params for model, params in model_configs.items() 
            if str(params.get("supported")).lower() == "true"
        }
        ...
        if new_parameters:
            model_configs[model_name]['parameters']['temperature'] = new_parameters.get(
                'temperature', model_configs[model_name]['parameters'].get('temperature')
            )
            model_configs[model_name]['parameters']['top_p'] = new_parameters.get(
                'top_p', model_configs[model_name]['parameters'].get('top_p')
            )
            model_configs[model_name]['parameters']['top_k'] = new_parameters.get(
                'top_k', model_configs[model_name]['parameters'].get('top_k')
            )
        ...
        # Return the full configuration dictionary for the model (not just its name)
